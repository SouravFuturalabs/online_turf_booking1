...
class Service {
  String url =
      "http://192.168.29.49/Online-Tuft-Management-Sxtreme--Chinmaya/API/";
  ...
  customerRegNew(String name, address, dob, phone, email, password,
      BuildContext context, File cutomerPhoto, File idproof) {
    EasyLoading.show(status: 'loading...');
    final fullurl = "${url}customer_reg.php";
    var request = MultipartRequest("POST", Uri.parse(fullurl));
    request.fields["Cname"] = name;
    request.fields["Caddress"] = address;
    request.fields["Cdob"] = dob;
    request.fields["CEmail"] = email;
    request.fields["CPassword"] = password;
    request.fields["CPhone_no"] = phone;
    request.files.add(MultipartFile.fromBytes(
        "customer_photo", File(cutomerPhoto.path).readAsBytesSync(),
        filename: cutomerPhoto.path));
    request.files.add(MultipartFile.fromBytes(
        "idproof", File(idproof.path).readAsBytesSync(),
 ...
 EasyLoading.dismiss();
  ...
  turfRegNew(
      ...
      BuildContext context) async {
    EasyLoading.show(status: 'loading...');
    final fullurl = "${url}turf_reg_new.php";
    var request = MultipartRequest("POST", Uri.parse(fullurl));
    ...
    request.files.add(MultipartFile.fromBytes(
        "license", File(licphoto.path).readAsBytesSync(),
        filename: licphoto.path));
    request.files.add(MultipartFile.fromBytes(
        "owner_photo", File(ownerPhtot.path).readAsBytesSync(),
        filename: ownerPhtot.path));
    ...
    request.send().then((response) async {
      if (response.statusCode == 200) {
        ...
        var rbody = jsonDecode(data.body);
        print("response of trf added${data.body}");
        if (rbody["message"] == "sucess") {
          SharedPreferences sharedPreferences =
              await SharedPreferences.getInstance();
          sharedPreferences.setString("id", rbody["Turf_id"]);
  ...
  Future<dynamic> bookturf(String turfid, bookingdate, bookingStatus, timeslot,
      daysno, amount) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    var id = await sharedPreferences.getString("id");
    var body = {
  ...
  Future<dynamic> getNotification() async {
    ...
    if (response.statusCode == 200) {
      print("response of notification ${response.body}");
      var rbody = jsonDecode(response.body);
  ...
  Future<dynamic> allRequest() async {
    ...
    var body = {"Turf_id": id};
    var response =
        await post(Uri.parse("${url}view_turfrequest.php"), body: body);
  ...
  Future<dynamic> acceptOrReject(String bookingid, status) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    var id = await sharedPreferences.getString("id");
    var body = {"Book_id": bookingid, "status": status};
    var response =
        await post(Uri.parse("${url}approve_turfrequest.php"), body: body);
    if (response.statusCode == 200) {
      var rbody = jsonDecode(response.body);
      return rbody;
    }
  ...
  Future<dynamic> history() async {
    ...
    var response =
        await post(Uri.parse("${url}view_turfhistory.php"), body: body);
    if (response.statusCode == 200) {
      var rbody = jsonDecode(response.body);
      print(rbody);
      if (rbody[0]["message"] != "failed") {
  ...
  Future<dynamic> paymenthistory() async {
    ...
    var response =
        await post(Uri.parse("${url}view_payhistory.php"), body: body);
  ...
  Future<dynamic> getPaumentUsingBookingID(String id) async {
    var body = {"book_id": id};
    var response = await post(Uri.parse("${url}view_payments.php"), body: body);
    if (response.statusCode == 200) {
      var body = jsonDecode(response.body);
      print(body);

      if (body[0]["message"] != "failed") {
        return body;
      } else {
        return false;
      }
  ...
  Future<dynamic> chnageTurfStatus(String id, status) async {
    var body = {"turf_id": id, "status": status};
    var response = await post(
        Uri.parse("${url}change_turf_avalibale_status.php"),
        body: body);
    if (response.statusCode == 200) {
      var body = jsonDecode(response.body);
      print("reponse of change statuks${body}");

      if (body["message"] == "sucess") {
        return body;
      } else {
        return false;
      }
